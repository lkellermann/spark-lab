...
APP_NAME = f"(Delta ZOrder) {__file__}"
...
def write_delta_then_zorder(spark):

    output = "/opt/spark/data/bronze/individual_incident_delta_zorder"
    spark.sparkContext.setJobGroup(APP_NAME, "Creating Delta table...")
    ...
    query = f"optimize delta.`{output}` zorder by state, date_HRF"

    spark.sparkContext.setJobGroup(APP_NAME, "Optimizing table with Zorder...")
    spark.sql(query)
