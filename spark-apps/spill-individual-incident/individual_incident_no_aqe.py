...
APP_NAME = f"(Spill - No AQE - 450m - 0.60) {__file__}"
...
def main():
    ...
    # make submit app=spill-individual-incident/individual_incident_no_aqe.py
    #
    ...
    spark.sparkContext.setJobGroup(APP_NAME, "Sorting and writing dataframe - 100 Partitions.")
    spark.conf.set("spark.sql.shuffle.partitions", 100)
    df_input.sort("ID", ascending = False).write.format("noop").mode("overwrite").save()

    spark.sparkContext.setJobGroup(APP_NAME, "Sorting and writing dataframe - 50 Partitions.")
    spark.conf.set("spark.sql.shuffle.partitions", 50)
    df_input.sort("ID", ascending = False).write.format("noop").mode("overwrite").save()


    spark.sparkContext.setJobGroup(APP_NAME, "Sorting and writing dataframe - 10 Partitions.")
